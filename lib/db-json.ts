...
async function ensureDataDir() {
    if (isVercel) {
        // On Vercel, skip directory creation
        return;
    }
...
async function readData<T>(filePath: string): Promise<T[]> {
    try {
        ...
        if (isVercel) {
            console.warn(`[Vercel] Cannot read ${filePath}. Using empty data. Please configure a database.`);
        }
        return [];
    }
}
...
async function writeData<T>(filePath: string, data: T[]): Promise<void> {
    if (isVercel) {
        throw new Error('Cannot write to filesystem on Vercel. Please configure a database (e.g., Vercel Postgres, MongoDB).');
    }
